...
} from "@/components/titap/Dropdown";
    ...
    <Dropdown.Root>
      <Dropdown.Trigger asChild>
        ...
        >
          ...
          />
          <Icon className="w-2 h-2" name="ChevronDown" />
        </Toolbar.Button>
      </Dropdown.Trigger>
          ...
          {options.map((option) => {
            if (isOption(option)) {
              return (
                ...
                >
                  <Icon className="w-4 h-4 mr-1" name={option.icon} />
                  {option.label}
                </DropdownButton>
              );
            } else if (isCategory(option)) {
              return (
                <div key={option.id} className="mt-2 first:mt-0">
